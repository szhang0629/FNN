source("source.R")
main <- function(seed, vari = 0, p = 200, D = 3, lambda. = 10^(-2:-1)) {
  ...
  if (vari == 0) {
    ...
    loc <- data.frame(PTID = rep(1:n, each = 20), loc = rep(runif(1:n), 20))
    ...
    ic <- 3
    Y$loc <- (bbz$params[length(bbz$params) - ic] - bbz$params[1 + ic]) * 
      (Y$loc - min(Y$loc)) / (max(Y$loc) - min(Y$loc)) + bbz$params[1 + ic] 
